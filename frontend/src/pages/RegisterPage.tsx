...
import { Loader2, AlertCircle, Activity, User, ShieldCheck } from "lucide-react";
                  ...
                  <CardContent className="space-y-6 px-8">
                    ...
                    <div className="space-y-5">
                      <motion.div variants={itemVariants} className="space-y-3">
                        <Label className="text-muted-foreground ml-1">I am a...</Label>
                      ...
                      <motion.div variants={itemVariants} className="space-y-2">
                        <Label htmlFor="name" className="text-muted-foreground ml-1">Display Name</Label>
                        <Input id="name" type="text" placeholder="e.g. Blagoja" value={name}
                      ...
                      <motion.div variants={itemVariants} className="space-y-2">
                        ...
                        <Input id="email" type="email" placeholder="hello@example.com" value={email}
                      ...
                      <motion.div variants={itemVariants} className="space-y-2">
                        <Label htmlFor="confirmPassword" className="text-muted-foreground ml-1">Confirm Password</Label>
                    ...
                    <motion.div variants={itemVariants} className="flex justify-center pt-8 pb-4">
                      <Button type="submit" disabled={isSubmitting}
                        ...
                        {isSubmitting ? <><Loader2 className="mr-2 h-5 w-5 animate-spin" />Creating...</> : "Register"}
                      </Button>
                    </motion.div>
                  </CardContent>

                  <CardFooter className="flex flex-col pb-8">
                    <motion.p variants={itemVariants} className="text-center text-sm text-muted-foreground w-full mt-2">
                      Already have an account?{" "}
                      <Link to="/login" className="text-primary hover:text-primary/80 transition-colors font-semibold">Log in</Link>
                    </motion.p>
                  </CardFooter>
                </form>
              </motion.div>
              ...
              >
                ...
                <form onSubmit={handleVerify}>
                  <CardContent className="space-y-6 px-8">
                    ...
                    <motion.div variants={itemVariants} className="flex justify-center gap-2 pt-2">
                      {digits.map((digit, i) => (
                        <Input
                          key={i}
                    ...
                    <motion.div variants={itemVariants} className="flex justify-center pt-4 pb-4">
                      <Button type="submit" disabled={isSubmitting}
                        className="w-2/3 h-12 rounded-full font-bold text-md shadow-[0_0_20px_rgba(var(--primary),0.2)] hover:shadow-[0_0_25px_rgba(var(--primary),0.4)] transition-all hover:-translate-y-0.5">
                        {isSubmitting ? <><Loader2 className="mr-2 h-5 w-5 animate-spin" />Verifying...</> : "Verify Email"}
                      </Button>
                    </motion.div>
                  </CardContent>
                  ...
                  <CardFooter className="flex flex-col pb-8">
                    <motion.p variants={itemVariants} className="text-center text-sm text-muted-foreground w-full mt-2">
                      Wrong email?{" "}
                      <button type="button" onClick={() => { setStep("register"); setError(null); setDigits(["","","","","",""]); }}
